...
class WSServer {
  constructor({ wsRoot, project, platform, port } = {}) {
  ...
  _WSConnected(ws, request) {
    ...
    // );
    const mockpath = path.resolve(`${this.wsRoot}${urlpath}.js`);
    const fileExists =
  ...
  _handleWSIncoming(
    ...
    parsedMessage.args.forEach((channel) => {
      let mockpath = path.resolve(
        `${this.wsRoot}${urlpath}/${channel}.js`
      );
      const dummy = path.resolve(
        `${this.wsRoot}${urlpath}/${subscribeMap[channel]}.js`
      );
